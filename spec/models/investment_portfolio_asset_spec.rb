...
RSpec.describe InvestmentPortfolioAsset, type: :model do
  ...
  describe 'validations' do
    ...
    it 'validates target_allocation_weight_percentage is greater than or equal to 0 and less than or equal to 100' do
      ...
      expect(investment_portfolio_asset.errors[:target_allocation_weight_percentage]).to include('must be greater than 0 and less than or equal to 100')
      expect(investment_portfolio_asset).to be_invalid
    end

    it 'validates target_variation_limit_percentage is greater than or equal to 0 and less than or equal to 100' do
      investment_portfolio_asset = build(:investment_portfolio_asset, target_variation_limit_percentage: 101)
      investment_portfolio_asset.valid?

      expect(investment_portfolio_asset.errors[:target_variation_limit_percentage]).to include('must be greater than 0 and less than or equal to 100')
      expect(investment_portfolio_asset).to be_invalid
    end
    ...
    it 'validates that :quantity is greater than or equal to 0' do
      ...
      expect(investment_portfolio_asset.errors[:quantity]).to include('must be greater than 0')
      expect(investment_portfolio_asset).to be_invalid
    end
  end
end
